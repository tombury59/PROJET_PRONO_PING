<aside
    ...
    class="fixed inset-y-0 left-0 z-30 flex shrink-0 flex-col border-r border-neutral-200 bg-white transition-all duration-200 ease-in-out lg:relative lg:translate-x-0 dark:border-neutral-800 dark:bg-neutral-950"
>
    <!-- Logo / brand -->
    <div class="flex h-16 shrink-0 items-center gap-2 border-b border-neutral-200 px-4 dark:border-neutral-800">
        <a href="{{ route('dashboard') }}" class="flex items-center gap-2 overflow-hidden">
            ...
            <span x-show="sidebarOpen" x-transition.opacity class="truncate font-semibold text-neutral-900 dark:text-white">
                {{ config('app.name') }}
            </span>
        </a>
    ...
    <nav class="flex-1 space-y-1 overflow-y-auto px-3 py-4">
        ...
        >
            ...
            <span x-show="sidebarOpen" x-transition.opacity class="truncate">Tableau de bord</span>
        </a>
    ...
    >
        ...
        <span x-show="sidebarOpen" x-transition.opacity>Réduire</span>
    </button>
    ...
    <div class="shrink-0 border-t border-neutral-200 p-3 dark:border-neutral-800">
        <div class="flex items-center gap-3 overflow-hidden px-1 py-1">
            ...
            <span x-show="sidebarOpen" x-transition.opacity class="truncate text-sm font-medium text-neutral-700 dark:text-neutral-300">
                {{ auth()->user()->pseudo ?? '' }}
            </span>
        </div>
        ...
        <form method="POST" action="{{ route('logout') }}" class="mt-1">
            ...
            >
                ...
                <span x-show="sidebarOpen" x-transition.opacity>Déconnexion</span>
            </button>
        </form>
    </div>
</aside>
